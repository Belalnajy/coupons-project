...
export function CommentItem({ comment, onEdit, onDelete }: CommentItemProps) {
  ...
  const badgeColor = comment.user.badge?.toLowerCase() === 'gold' ? 'bg-gold' : 'bg-silver';
        ...
        <div className="flex-1 space-y-2 min-w-0">
          <div className="flex items-center gap-2 flex-wrap justify-between">
            <div className="flex items-center gap-2">
              ...
              {comment.user.badge && (
                <Badge className={`${badgeColor} text-[10px] py-0 px-2 h-5 text-black border-none`}>
                  {comment.user.badge}
                </Badge>
              )}
            ...
            {/* Action Menu (Visible on hover for current user) */}
            {!isEditing && (
          ...
          {isEditing ? (
            <div className="space-y-3 pt-1">
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  <FiCheck className="mr-1" /> {isUpdating ? "Saving..." : "Save"}
                </Button>
              </div>
            </div>
          ) : (
          ...
          <div className="flex items-center gap-4 pt-2">
            <button className="flex items-center gap-1 text-xs text-light-grey hover:text-white transition-colors cursor-pointer group/like">
              <FiThumbsUp className="w-3 h-3 group-hover/like:scale-110 transition-transform" /> {comment.likes}
            </button>
